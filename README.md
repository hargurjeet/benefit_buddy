...
3. **Live Verification**: A concurrent web search fetches up-to-date details or application portals directly from official sites.
4. **Strict URL & Hallucination Guardrails**: Outbound links are strictly limited to official government domains (`.gov.in`, `.nic.in`), and LLM recommendations are dynamically verified against retrieved sources to eliminate hallucinations.
...
- **Role**: Coordinates two sub-agents concurrently using `ParallelAgent`.
...
- **Performance Benefit**: Using `ParallelAgent` reduces the latency of executing both slow search tools from sequential (T1 + T2) to parallel (max(T1, T2)).
...
- **Role**: Synthesizes the demographic profile, database search results, and web search updates to output eligibility guidelines.
...
- **Hallucination Pruning**: Every recommended scheme section must match keywords present in the search results (`dataset_search_result` or `web_search_result`). If the LLM generates a recommendation not validated by the tools, the section is pruned, and a warning is printed.
